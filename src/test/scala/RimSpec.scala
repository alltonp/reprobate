...
//  }
  ...
  "add issue" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(0))))
  ...
  "add and move forward to begin state" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(1))))
    runAndExpect("+/ an item", current, expected)
  }

  "add and move forward to second state" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(2), by = Some(aka))))
    runAndExpect("+// an item", current, expected)
  }

  "add and move forward to end state" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(config.lastWorkflowStateIncludingPre), by = Some(aka))))
    runAndExpect("+! an item", current, expected)
  }

  "add with tags" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(0), tags = Some(Set("tag1", "tag2")))))
  ...
  "add and move forward to begin state with tags" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(1), tags = Some(Set("tag1", "tag2")))))
    runAndExpect("+/ an item : tag1 tag2", current, expected)
  }

  "add and move forward to end state with tags" in {
    val current = emptyModelWithWorkflow
    val expected = current.copy(issues = List(Issue("1", "an item", Some(config.lastWorkflowStateIncludingPre), by = Some(aka), tags = Some(Set("tag1", "tag2")))))
    runAndExpect("+! an item : tag1 tag2", current, expected)
  }
  ...
  "edit issue retains by, tags and status" in {
    val issue = Issue("1", "an item", Some(2), by = Some(aka), tags = Some(Set("tag1", "tag2")))
    val current = modelWithIssue(issue)
    val expected = current.copy(issues = List(Issue("1", "an item edited", Some(2), by = Some(aka), tags = Some(Set("tag1", "tag2")))))
  ...
  "edit with tags adds tags" in {
    ...
    val issue = Issue("1", "an item", Some(2), by = Some(aka), tags = Some(Set("tag1", "tag2")))
    val current = modelWithIssue(issue)
    val expected = current.copy(issues = List(Issue("1", "an item edited", Some(2), tags = Some(Set("tag1", "tag2", "tags3")))))
  ...
  "move forward one state" in {
    val issue = Issue("1", "an item", Some(2))
    val current = modelWithIssue(issue)
...
//  }
  ...
  "move forward to an initial leaves disowned" in {
    val issue = Issue("1", "an item", Some(0))
    val current = modelWithIssue(issue)
  ...
  "move back a state" in {
    val issue = Issue("1", "an item", Some(config.lastWorkflowStateIncludingPre))
    val current = modelWithIssue(issue)
    val expected = current.copy(issues = List(issue.copy(status = Some(2), by = Some(aka))))
    runAndExpect("1 .", current, expected)
  }
  ...
  "move back a state (into preWorkflowState)" in {
    val issue = Issue("1", "an item", Some(1))
    val current = modelWithIssue(issue)
...
//    Some(Model(Config(rim,backlog,List(next, doing, done),released,List()),Map(anon -> A, anon2 -> B),List(Issue(1,an item,None,None,None,None,Set())),List())) did not equal
  ...
  "set values merges, overwriting newer values" in {
    val issue = Issue("1", "an item", Some(2), values = Some(Map("key1" -> "value1", "key2" -> "value2")))
    val current = modelWithIssue(issue)
  ...
  "last unset values results in None" in {
    val issue = Issue("1", "an item", Some(2), values = Some(Map("key1" -> "value1")))
    val current = modelWithIssue(issue)
...
//  }
  ...
  "move x under y" in {
    val issue1 = Issue("1", "an item", Some(2))
    val issue2 = Issue("2", "an item", Some(2))
    val issue3 = Issue("3", "an item", Some(2))
  ...
  "add another comment" in {
    val issue = Issue("1", "an item", Some(2), comments = Some(List("a comment")))
    val current = modelWithIssue(issue)
  ...
  "unblock" in {
    val issue = Issue("1", "an item", Some(1), blocked = Some("waiting on x"))
    val current = modelWithIssue(issue)
  ...
  "disown" in {
    val issue = Issue("1", "an item", Some(1), by = Some(aka))
    val current = modelWithIssue(issue)
  ...
  "detag" in {
    val issue = Issue("1", "an item", Some(1), tags = Some(Set("tag")))
    val current = modelWithIssue(issue)
  ...
  "migrate tag" in {
    val issue = Issue("1", "an item", Some(1), tags = Some(Set("tag1", "tag2", "tagN")))
    val current = modelWithIssue(issue)
  ...
  "migrate tag in released" in {
    val issue = Issue("1", "an item", Some(config.lastWorkflowStateIncludingPre), tags = Some(Set("tag1", "tag2", "tagN")))
